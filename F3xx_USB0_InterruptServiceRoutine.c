				...
				if(Feature_report_buffer[0] == 1)
				{
					switch(Feature_report_buffer[1])
					{
						case 0x30:
							...
							{
								case 0x0F:
									spi_write_sensor(Feature_report_buffer[2], Feature_report_buffer[3]);
									resolution = Feature_report_buffer[3];										//load default resolution
									spi_write_eeprom(0x0000, resolution);
							...
							break;
						case 0x31:
							Feature_report_buffer[3] = spi_read_sensor(Feature_report_buffer[2]);
							break;
						case 0x33:
							image_buffer[1] = 0;
							grab_image = 1;
						...
						case 0x34:
							...
							break;
						case 0x40:
//							spi_write_eeprom(Feature_report_buffer[2], Feature_report_buffer[3]);
							break;
						case 0x41:
//							Feature_report_buffer[3] = spi_read_eeprom(Feature_report_buffer[2]);
							break;
						case 0x50:
//							FLASH_ByteWrite (0x3A00 + Feature_report_buffer[2], Feature_report_buffer[3]);
							break;
						case 0x51:
//							Feature_report_buffer[3] = FLASH_ByteRead(0x3A00 + Feature_report_buffer[2]);
							break;
						case 0x52:
//							Feature_report_buffer[3] = 0x52;
//							FLASH_PageErase(0x3A00 + Feature_report_buffer[2]);
							break;
						case 0x71:
						...
						case 0x80:
							...
							{
								case 0x01:
									mouse_enabled = Feature_report_buffer[3];
									Feature_report_buffer[3] = 0x01;
									break;
								default:
									Feature_report_buffer[3] = 0x00;
									break;
							}
							break;
						case 0x81:
							...
							{
								case 0x01:
									Feature_report_buffer[3] = mouse_enabled;
									break;
								default:
									Feature_report_buffer[3] = 0x00;
									break;
						...
						case 0xA0: 	// 2nd Step	// Write Data	
							FLASH_ByteWrite((SROM_TEMP_ADDR + srom_addr), Feature_report_buffer[3]);
							crc_return = Eight_Bit_CRC(crc_return, Feature_report_buffer[3]);
							srom_addr++;
							break;
						case 0xA1:	// 3rd Step	// Read CRC
							Feature_report_buffer[3] = crc_return;
							break;
						case 0xA2:	// 1st Step	// Erase	
							WDT_Disable();
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0000);
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0200);
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0400);
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0600);
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0800);
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0A00);
							FLASH_PageErase(SROM_TEMP_ADDR + 0x0C00);							

							srom_addr = 0;
							crc_return = 0;
							srom_start = 1;
							break;
						case 0xA3:	// 4nd Step	// Set download status
							FLASH_ByteWrite ((SROM_TEMP_ADDR + 0x0C02), Feature_report_buffer[3]);
							if(Feature_report_buffer[3] == 0x01)
							{
								srom_size.w = srom_addr;
								FLASH_ByteWrite ((SROM_TEMP_ADDR + 0x0C00), srom_size.b[0]);
								FLASH_ByteWrite ((SROM_TEMP_ADDR + 0x0C01), srom_size.b[1]);
								srom_success = 1;
							}
							else
							{
