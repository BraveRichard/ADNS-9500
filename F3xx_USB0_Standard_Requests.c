...
extern hid_configuration_descriptor 	HIDCONFIGDESC1;
extern unsigned char 			HIDREPORTDESC1;
...
extern data unsigned char calibration_mode;
...
#define ConfigDesc1 	(HIDCONFIGDESC1.hid_configuration_descriptor)
#define InterfaceDesc1 	(HIDCONFIGDESC1.hid_interface_descriptor)
#define HidDesc1 		(HIDCONFIGDESC1.hid_descriptor)
#define Endpoint1Desc1 	(HIDCONFIGDESC1.hid_endpoint_in_descriptor)
#define Endpoint2Desc1 	(HIDCONFIGDESC1.hid_endpoint_out_descriptor)
//-----------------------------------------------------------------------------
// Get_Status
//-----------------------------------------------------------------------------
//
...
{                                      // and size to correct descriptor and
   ...
   {                                   // was requested, and set data ptr and
      ...
      case DSC_CONFIG:
		 ...
		 DATAPTR = (unsigned char*) &ConfigDesc1;
		else
      ...
      case DSC_INTERFACE:
	  	if(calibration_mode == 1)
		 DATAPTR = (unsigned char*) &InterfaceDesc1;
		else
      ...
      case DSC_ENDPOINT:
         if ( (SETUP.wValue.c[LSB] == IN_EP1) )
         {
            ...
            {
			  	if(calibration_mode == 1)
				 DATAPTR = (unsigned char*) &Endpoint1Desc1;
				else
            ...
            {
			  	if(calibration_mode == 1)
				 DATAPTR = (unsigned char*) &Endpoint2Desc1;
				else
	...
	case DSC_HID: // HID Specific (HID class descriptor)
	  	if(calibration_mode == 1)
		 DATAPTR = (unsigned char*) &HidDesc1;
		else
	...
	case DSC_HID_REPORT: // HID Specific (HID report descriptor)
	  	if(calibration_mode == 1)
		{
			DATAPTR = (unsigned char*)&HIDREPORTDESC1;
			DATASIZE = HID_REPORT_DESCRIPTOR1_SIZE;
		}
		else
		{
...
*/
	...
	if(calibration_mode == 1)
	{
		if (   (SETUP.wValue.c[MSB] == HID_OUTPUT_REPORT )        // Set_Report( Feature )     
		    && (SETUP.wValue.c[LSB] == 0)                   // default report ID     
			&& (SETUP.wIndex.i      == 0)                   // Interface 0    
			&& (SETUP.wLength.i     == 3 ) ) // report size    
		{        
			DATASIZE = 3;
			DATAPTR  = OUT_PACKET;        // Set serviced SETUP Packet        
			POLL_WRITE_BYTE (E0CSR, rbSOPRDY);        
			EP_STATUS[0] = EP_RX;                   // Endpoint 0 in transmit mode
			DATASENT = 0;                           // Reset DATASENT counter
	    } 
